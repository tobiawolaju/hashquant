...
export async function initDb(): Promise<boolean> {
  if (!pool) {
    console.warn('DATABASE_URL is not configured; database-backed endpoints will be unavailable.');
    return false;
  }
  ...
  try {
    ...
    console.warn('Database initialization failed; database-backed endpoints will be unavailable.', error);
